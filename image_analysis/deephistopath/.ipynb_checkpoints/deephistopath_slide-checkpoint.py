...
SCALE_FACTOR = 32
THUMBNAIL_SIZE = 300
BASE_DIR = "working/lupolab/pranathi/SPORE-DRP/Scripts/preprocess-data/"
STATS_DIR = "WSI_processing/svs_stats"
DEST_PNG_DIR = "WSI_processing/png_images"
DEST_THUMBNAIL_DIR =  "WSI_processing/thumbnail_images"#os.path.join(BASE_DIR, "thumbnail_images")
...
def singleprocess_training_slides_to_images(imgs_paths=None):
    """
    ...
    if not os.path.exists(DEST_THUMBNAIL_DIR):
        os.mkdir(DEST_THUMBNAIL_DIR)
    
    num_images = len(imgs_paths)   
    for slide_path in imgs_paths:
        
        slide_name = slide_path.split("/")[-1].split(".")[0] # slide names 
        img, large_w, large_h, new_w, new_h = slide_to_scaled_pil_image(slide_path) # get img ,width ,height
        
        img_path = os.path.join(DEST_PNG_DIR, slide_name + "-" + str(SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(large_h) + "." + "png")
        print("Saving image to: " + img_path)
        img.save(img_path)

        thumbnail_path = os.path.join(DEST_THUMBNAIL_DIR, slide_name + "-" + str(SCALE_FACTOR) + "x-" + str(large_w) + "x" + str(large_h) +
                                      "." + "jpg")
