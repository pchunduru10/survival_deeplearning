...
#         return data_dict #[v for v in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)]#if v.name.endswith('weights:0')
    

def load_pretrained_weights(weights_path, session, skip_layers=[]):
        if weights_path is not None:
            print("Loading pretrained weights")
#             weights_path =  self.weights_path +'ResNet-L{}.npy'.format(self.depth)
